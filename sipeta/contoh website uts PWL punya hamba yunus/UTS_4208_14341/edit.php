<?php
include "conn.php";
$sql2 = "SELECT * FROM link_yt order by id ASC";
$result = mysqli_query($conn, $sql2);
$data1 = mysqli_fetch_assoc($result);
$id_list = $data1["id_list"];

if ($id_list > 0) {
    $sql = mysqli_query($conn, "Select * from link_list where id='$id_list'");
    $data = mysqli_fetch_array($sql);
} else {
    $sql = mysqli_query($conn, "Select * from link_list where id='$_GET[id]'");
    $data = mysqli_fetch_array($sql);
}
...
if (isset($_POST['proses'])) {
    ...
    note = '$_POST[note]',
    link = '$_POST[link]' where id_list='$id_list'") or die(mysqli_error($conn));

    echo "<script>document.location='save-link.php'</script>";
}
